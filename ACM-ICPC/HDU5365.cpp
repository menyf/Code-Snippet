	...
	while (scanf("%d",&n)!=EOF) {
		...
		for(int i =1 ; i <= n ; i++){
			for(int j = 1; j <= n ; j++){
				...
				for(int k = 1; k <= n; k++){
					if(i == k || j == k) continue;
					line l1 = line(p[k], p[i]);
					line l2 = line(p[j], p[i]);
					if(fabs(l1.norm() - l2.norm()) < eps){
						if (fabs(dot(l1.vec(), l2.vec())) < eps) {
							point mid = (p[k] + p[j]) / 2.0f;
							point tmp = 2.0 * mid - p[i];
							int x = tmp.x + eps;
							int y = tmp.y + eps;
							if (mp[x][y]) {
								ans++;
